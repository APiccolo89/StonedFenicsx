...
import solver_function.compute_material_property as cmp
...
@njit
def _compute_lithostatic_pressure(nz,option_rho,g,dz,T):
    ...
    while res>1e-6:
        for i in range(0,nz):
            rho = cmp.density(option_rho,T[i],lit_p[i],1)
            rhog = rho*g
            if i == 0:
                lit_p[i] = 0.0
            else: 
...
#-----------------------------------------------------------------------------------------
@njit
def _compute_Cp_k_rho(option_Cp,option_k,option_rho,Cp,k,rho,T,lit_p):
    it = len(T)

    for i in range(it):
        k[i] = cmp.heat_conductivity(option_k,T[i],lit_p[i],1)
        Cp[i] = cmp.heat_capacity(option_k,T[i],lit_p[i],1)
        rho[i] = cmp.density(option_rho,T[i],lit_p[i],1)

    return Cp,k,rho
#-----------------------------------------------------------------------------------------
@njit
def build_coefficient_matrix(A,B,D,Q,M,ctrl,lhs,TO,TG,TPr,k_m,density_m,heat_capacity_m,step,lit_p,sc):

    option_k   = ctrl.option_k 
    option_rho = ctrl.option_rho
    option_Cp  = ctrl.option_Cp
    nz         = lhs.nz 
    Ttop       = ctrl.Ttop*sc.Temp 
    Tmax       = ctrl.Tmax*sc.Temp
    dt         = lhs.dt 
    dz         = lhs.dz
    ...
    if step == 0:
        ...
        heat_capacity_m,k_m,density_m=_compute_Cp_k_rho(option_Cp,option_k,option_rho,heat_capacity_m,k_m,density_m,TG,lit_p)
        ...
        Cp_m0,k_m0,rho_m0=_compute_Cp_k_rho(option_Cp,option_k,option_rho,heat_capacity_m,k_m,density_m,TG,lit_p)

        Cp_m1,k_m1,rho_m1=_compute_Cp_k_rho(option_Cp,option_k,option_rho,heat_capacity_m,k_m,density_m,TPr,lit_p)
        ...
        for j in range(start_loop,end_loop+1): 
            ...
            if (i == 0 and j == 0): 
                ...
                if j > 0 and j < nz-1:
                    ...
                    rho_A =  cmp.density(option_rho,TO[j],lit_p[j],1)
                    Cp_A  = cmp.heat_capacity(option_Cp,TO[j],lit_p[j],1)
            
                    if step == 0:
                        
                        rho_B = cmp.density(option_rho,TO[j],lit_p[j],1)
                        Cp_B = cmp.heat_capacity(option_Cp,TO[j],lit_p[j],1)

                        # B - predictor step 
                        B[j] = -TO[j] * ( rho_A * Cp_A - rho_B * Cp_B) / (rho_A * Cp_A)

                    elif step == 1: 
                        rho_B = cmp.density(option_rho,TPr[j],lit_p[j],1)
                        Cp_B = cmp.heat_capacity(option_Cp,TPr[j],lit_p[j],1)
...
#-----------------------------------------------------------------------------------------
def compute_initial_LHS(ctrl,lhs,scal):
    ...
    alpha_g    = lhs.alpha_g
    g          = ctrl.g*scal.ac
    Tmax       = ctrl.Tmax*scal.Temp
    Ttop       = ctrl.Ttop*scal.Temp
    option_k   = ctrl.option_k
    option_Cp  = ctrl.option_Cp
    option_rho  = ctrl.option_rho
    ...
    Told = zeros((nz))
    for i in range(0,nz):
        slope = (alpha_g * g) / cmp.heat_capacity(option_Cp,Told[i-1],0,1)
    
        if z[i] == 0:
            Told[i] = Ttop + 0 
        elif z[i] < depth_melt:  
            Tdepth_melt = slope * depth_melt + Tmax  - slope * z[-1]
            Told[i] = Told[i-1] + Tdepth_melt/(index_depth_melt+1)
        else:
            Told[i] = slope * z[i] + Tmax  - slope * z[-1] 


    lit_p = _compute_lithostatic_pressure(nz,option_rho,g,dz,Told)
    ...
    for time in range(1,nt):
        ...
        while res > 1e-12:
            for step in range(2):   
                    ...
                    M,D = build_coefficient_matrix(A,B,D,Q,M,ctrl,lhs,TO,TG,TPr,k_m,density_m,heat_capacity_m,step,lit_p,scal)
            ...
            if np.isnan(res) == True:
                ...
                sys.exit(1)
            TG = Tnew 
            lit_p = _compute_lithostatic_pressure(nz,option_rho,g,dz,Tnew)
            it += 1
    ...
    if visualisation_1D == 1:
        ...
        X, Y  = np.meshgrid(t/1e6/364.25/24/60/60, z/1e3)     # make a mesh of X and Y for easy plotting in the right units (i.e., t in Myr and z in km) 
