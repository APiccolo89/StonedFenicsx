...
To run the numerical code, the user has to specify the test name and the path of the folder in which the output is stored. *StonedFEniCSx* automatically creates the parent folder, and the relative test folder. The code creates a **Cached_information** folder in which to store the mesh data and thermal boundary-condition data. The user can decide to always compute the mesh information and thermal boundary conditions, especially whenever the user needs to systematically change the geometry, or the thermal parameters.
...
path_test = Path(\_\_file\_\_).resolve().parents[0]
