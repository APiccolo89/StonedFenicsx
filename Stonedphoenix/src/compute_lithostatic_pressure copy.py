...
def _solve_lithostatic_pressure(M : Mesh , S : Solution ,ctrl : NumericalControls, ioctrl : IOControls , pdb : PhaseDataBase, sc : Scal):
    ...
    if rank == 0:
        ...
        print(f"Lithostatic pressure calculation: {timer-timerB :.2f} seconds")
...
def unit_test_pressure():
    ...
    create_output_debug(S, M, sc, pdb)
    # create vtk file and test wheter or not is good 
    pass

def create_output_debug(S, M, sc, pdb):
    
    mesh2 = M.mesh
    mesh2.geometry.x[:] *= sc.L 
    
    space_vis = fem.functionspace(mesh2,('CG',1))
    mesh      = space_vis.mesh
    
    u         = fem.Function(space_vis)
    u.interpolate(S.P_L) 
    
    ph  = fem.Function(space_vis)
    ph.interpolate(M.phase)
    
    v = ufl.TestFunction(space_vis)
    rho = fem.Function(space_vis)
    rho_trial = ufl.TrialFunction(space_vis)
    rho_expr = density_FX(pdb, M.T_i, u, M.phase,mesh2)
    ...
    if rank == 0: 
        ...
        fig = plt.figure()
        x = space_vis.mesh.geometry.x 
        triang = tri.Triangulation(x[:,0]/1e3,x[:,1]/1e3)
        plt.tripcolor(triang, u.x.array*sc.stress/1e9, shading='gouraud', cmap='viridis')
        ...
        x = space_vis.mesh.geometry.x 
        triang = tri.Triangulation(x[:,0]/1e3,x[:,1]/1e3)
        plt.tripcolor(triang, rho.x.array, shading='gouraud', cmap='seismic')
        ...
        x = space_vis.mesh.geometry.x 
        triang = tri.Triangulation(x[:,0]/1e3,x[:,1]/1e3)
        plt.tripcolor(triang, ph.x.array, shading='gouraud', cmap='inferno')
