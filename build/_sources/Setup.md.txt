...
- {math}`trench` coordinate of the trench in meters. 
...
The parametrised slab surface is computed using {math}`d\ell` and a function that describe {math}`\theta(\ell0)`. The function that describe the evolution of the bending angle as a function of the total distance from the trench ({math}`\ell`) is: 
...
```{math}
\begin{aligned}
x_{\text{new}} &= x_{\text{trench}} + d\ell \cos(\theta), \\
y_{\text{new}} &= y_{\text{trench}} - d\ell \sin(\theta).
\end{aligned}
```
...
```{math}
\begin{aligned}
x_{oc|bot} &= x_{slab} - tk_{slab|oc} \sin(\theta), \\
y_{oc|bot} &= y_{slab} - tk_{slab|oc} \cos(\theta).
\end{aligned}
```
where {math}`x_{oc|bot}` and {math}`y_{oc|bot}` are the {math}`x` and {math}`y` coordinate of either the oceanic moho or the bottom of the subducting plate, while {math}`x_{slab}` and {math}`y_{slab}` are the coordinate of the top-surface of the slab. {math}`tk_{slab|oc}` is the thickness of either the subducting plate or the oceanic crust. 
