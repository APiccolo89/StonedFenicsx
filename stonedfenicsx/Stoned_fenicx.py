...
from stonedfenicsx.utils import Input,Phase

dict_options = {"NoShear": 0, "Linear": 1, "SelfConsistent": 2}
dict_stokes = {"Direct": np.int32(1), "Iterative": np.int32(0)}


def generate_phase_database(IP:Input, Phin:Phase) -> PhaseDataBase:
...
def fill_geometrical_input(IP) -> Geom_input:
    ...
    if IP.van_keken and not IP.sub_constant_flag:
        raise ValueError(
            "Error: Van Keken Benchmarks cannot run with a variable bending angle"
        )
        ...
        if (f.name != "theta_out_slab") and (f.name != "theta_in_slab"):
            ...
            if not isinstance(values, np.ndarray):
                if isinstance(values, str):
                    ...
                    if (
                        f.name == "sub_theta_0"
                        or f.name == "sub_theta_max"
                        or f.name == "sub_constant_flag"
                    ):
                        string_2_print = (
                            f"          {it}. {f.name:s} = {values:.3f} [n.d.]"
                        )
            else:
            ...
            if f.name == "y":
                if g_input.y[0] >= 0.0:
                    raise ValueError(
                        f"minimum coordinate of y = {g_input.y[0]:.3f} is positive or equal to 0.0. The coordinate must be negative"
                    )
            if (
                (f.name == "ocr")
                or (f.name == "cr")
                or (f.name == "lc")
                or (f.name == "decoupling")
                or (f.name == "lit_mit")
                or (f.name == "lab_d")
                or (f.name == "trans")
                or (f.name == "resolution_normal")
                or (f.name == "resolution_refine")
                or (f.name == "decoupling")
                or (f.name == "slab_tk")
            ):
                ...
                if values < 0:
                    raise ValueError(
                        f"{f.name :s} is negative. The coordinate must be positive"
                    )
        it = it + 1
...
def configure_simulation(IP:Input,Ph_input):
    
    
    return ctrl,phase_data_base,Mesh

def stoned_fenicsx(IP, Ph_input):
    """Initialize and configure numerical controls for the Stoned FEniCSx simulation.

    Parameters
    ----------
    IP : object
        Input parameters object containing simulation configuration.
    Ph_input : object
    ...
    -------
    NumericalControls
        Configured numerical controls object for the simulation.
    """
    ...
    # IO controls
    io_ctrl = IOControls(test_name=IP.test_name, path_save=IP.path_test, sname=IP.sname)
    io_ctrl.generate_io()
    # Scaling parameters
    ...
    t_lhs = time_dependent_evolution(
        constant_age=IP.constant_age,
    ...
    phase_db = generate_phase_database(IP, Ph_input)
    # ---
    # Create mesh
    ...
    phase_db = _scaling_material_properties(phase_db, sc)
    ...
    solution_routine(M, ctrl, lhs, t_lhs, phase_db, io_ctrl, sc)
