...
def curve_fitting(xd:NDArray[float],yd:NDArray[float],path:str , name:str,max_depth:float)->NDArray[float]:
    ...
    if np.min(yd) > max_depth: 
        mind = np.min(yd)
    else: 
        mind = max_depth 

    slab_top,theta_mean, _ = create_slab_surface(f_theta,mind,stp = 5.0)
...
def read_file_slab(file_path:str,max_depth:float)->tuple[NDArray[float],NDArray[float]]:
    ...
    slab_top, theta_mean = curve_fitting(ax,ay,path.parent,path.stem,max_depth)
