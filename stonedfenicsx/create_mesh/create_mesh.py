...
def create_gmesh(ioctrl: IOControls, g_input: GeomInput, ctrl: NumericalControls):
    ...
    The function also updates the `GeomInput` object by computing and storing
    ...
    if van_keken:
        # This is hardcoded portion ensure that the geometry of van keken are respected.
        min_x = 0.0
        max_x = 660.0
        max_y = 0.0
        min_y = -600.0
    ...
    # Hard coded value: this ensure that the geometries are consistent with each-other.
    g_input.x[1] = slab_x[-1] + 60
    min_x = g_input.x[0]  # The beginning of the model is the trench of the slab
    max_x = g_input.x[1]
...
def create_domain_C(mesh_model, CP, LC, g_input):
    ...
    if g_input.cr != 0:
        ...
        if g_input.lc != 0:
            ...
            index_a = find_line_index(
                LC.lines_S, CP.coord_sub, (1 - g_input.lc) * g_input.cr
            )
            ...
            index_a = 0
            index_b = (
                find_line_index(LC.lines_S, CP.coord_sub, (1 - g_input.lc) * g_input.cr)
                - 1
            )
            buf_array = -LC.lines_S[2, index_a : index_b + 1]
            buf_array = buf_array[::-1]
...
def create_physical_line(
    CP: Class_Points, LC: Class_Line, g_input: GeomInput, mesh_model: gmsh.model
) -> gmsh.model:
    ...
    mesh_model.addPhysicalGroup(
        1, LC.tag_L_R[0 : i + 1], tag=dict_tag_lines["Right_lit"]
    )
    ...
    mesh_model.addPhysicalGroup(
        1, LC.tag_L_sub[0 : i + 1], tag=dict_tag_lines["Subduction_top_lit"]
    )

    mesh_model.addPhysicalGroup(
        1, LC.tag_L_sub[i:], tag=dict_tag_lines["Subduction_top_wed"]
    )
    ...
    if g_input.cr != 0:
        ...
        if g_input.lc != 0:
            mesh_model.addPhysicalGroup(
                1, LC.tag_L_Lcr, tag=dict_tag_lines["LCrust_overplate"]
            )
...
def create_gmsh(
    ...
    g_input: GeomInput,
) -> gmsh.model:  # geometry input class
    """
    ...
    if g_input.ocr != 0.0:
        Left_side_of_subduction_surf = gmsh.model.geo.addPlaneSurface(
            [10], tag=100
        )  # Left side of the subudction zone
        Oceanic_Crust_surf = gmsh.model.geo.addPlaneSurface(
            [15], tag=150
        )  # Left side of the subudction zone
    else:
        Left_side_of_subduction_surf = gmsh.model.geo.addPlaneSurface(
    ...
    Wedge = gmsh.model.geo.addPlaneSurface(
        [20], tag=200
    )  # Right side of the subudction zone
    Lithhospheric_Mantle_surf = gmsh.model.geo.addPlaneSurface(
        [25], tag=250
    )  # Right mantle
    if g_input.cr != 0:
    ...
    if g_input.ocr != 0.0:
        mesh_model.addPhysicalGroup(
            2, [Oceanic_Crust_surf], tag=dict_surf["oceanic_crust"]
        )
    mesh_model.addPhysicalGroup(2, [Wedge], tag=dict_surf["wedge"])
    mesh_model.addPhysicalGroup(
        2, [Lithhospheric_Mantle_surf], tag=dict_surf["overriding_lm"]
    )
    if g_input.cr != 0:
        mesh_model.addPhysicalGroup(2, [Crust_UC_surf], tag=dict_surf["upper_crust"])
        if g_input.lc != 0:
            mesh_model.addPhysicalGroup(
                2, [Crust_LC_surf], tag=dict_surf["lower_crust"]
            )
...
def create_subdomain(
    ...
    if name == "Subduction":
        ...
        specs = [
            ...
            ([3], 3),
            ([9], 4),
        ]  # [1] is the top, [2] is the right side of the wedge, [3] is the bottom side of the wedge, [4] is the left side of the wedge
        dict_local = {
